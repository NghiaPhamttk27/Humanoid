...
def train(env, sb3_algo, model_path=None, total_timesteps=0):
    if model_path is not None:
        match sb3_algo:
            case 'SAC':
                model = SAC.load(model_path, env)
                print(model_path, total_timesteps)
            case 'TD3':
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--path', help='Path to the saved model')
    parser.add_argument('-ts', '--timesteps', type=int,
                        help='Total number of training steps')
    args = parser.parse_args()

    if args.train:
        gymenv = gym.make(args.gymenv, render_mode=None)
        train(gymenv, args.sb3_algo, args.path, args.timesteps)
